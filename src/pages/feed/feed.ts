 ...
 */
  newExpression(){
    this.feedExpressionService.getExpressionForThatDay(this.dataForFeedEntryPage.selectedDate)
      .then(data=>{
        if(data != null){
          this.feedExpressions = this.feedExpressionService.appendNewRecordAndReturn(this.feedExpressions, this.dataForFeedEntryPage.babyCode,
            this.dataForFeedEntryPage.selectedDate, data.babyWeight);
            setTimeout( data => this.toggleGroup(this.feedExpressions[0]), 100);
        }else{
          this.feedExpressions = this.feedExpressionService.appendNewRecordAndReturn(this.feedExpressions, this.dataForFeedEntryPage.babyCode,
            this.dataForFeedEntryPage.selectedDate, null);
            setTimeout( data => this.toggleGroup(this.feedExpressions[0]), 100);
        }
      })
  ...
  datePickerDialog(feedExp: IFeed){
    ...
    androidTheme: this.datePicker.ANDROID_THEMES.THEME_HOLO_LIGHT
      ...
      date => {
        ...
        this.feedExpressionService.getExpressionForThatDay(feedExp.dateOfFeed)
      .then(data=>{
        if(data != null){
          feedExp.babyWeight = data.babyWeight;
        }else{
          feedExp.babyWeight = null;
        }
      })
      },
